...
async fn me_with_garbage_token_is_unauthorized() {
    let app = TestApp::spawn().await;
    let (status, _) = app
        .get_with("/api/v1/auth/me", Some("not-a-real-token"))
        .await;
    assert_eq!(status, StatusCode::UNAUTHORIZED);
}
...
async fn nonce_rejects_malformed_wallet_address() {
    ...
    let (status, _) = app
        .post(
            "/api/v1/auth/nonce",
            serde_json::json!({ "wallet_address": "not-real" }),
        )
        .await;
...
async fn verify_rejects_a_replayed_nonce() {
    ...
    let address =
        zenith_backend::strkey::encode_stellar_public_key(signing_key.verifying_key().as_bytes());
